#include "Arduino.h"  //  
#include "Wire.h"     //  Biblioteca de comunicação I2C
#include <ESP8266WiFi.h>
#include <PubSubClient.h>

WiFiClient espClient;
PubSubClient client(espClient);

/*
  ================================================================
  ===                          PINOUT                          ===
  ================================================================
*/

//MPU
/*VCC       3v
  GND       GND
*/
#define scl_pin  D6
#define sda_pin  D5

//Botão
//uint8_t botOffset = D4;X

//Leds
#define LED1 D0
#define LED2 D1

/*
  ===============================================================
  ===                        VARIÁVEIS                        ===
  ===============================================================
*/

// Tempo
double dt, tempo_prev = 0.00;

// Armazenamento dos dados "crus" do acelerômetro
int16_t AX, AY, AZ, Tp, GX, GY, GZ;

// Variavel do offset
bool OFFSET = false;
double offAX = 0, offAY = 0, offAZ = 0;

// Armazenamento dos dados após conversão
double AcX, AcY, AcZ, Tmp, GyX, GyY, GyZ;
long convGYRO, convACEL;     // Datasheet do sensor

// Calculo ângulos
double Pitch = 0;
double Roll = 0;
double Yaw = 0;
double Acc1;
double Acc2;
#define RAD_TO_DEG 57.2957795  // Radianos para graus (180/PI)
double erroROLL   = -2.951641908101069;
double erroPITCH  = -3.908116257882335;
double erroYAW    = 0.793767901287199;

// Calculo velocidade
double VelX = 0, VelY = 0, VelZ = 0;
double erroVZ = 0.11 / 15530;

// Media movel
double AmX = 0, AmY = 0, AmZ = 0;
#define n 30

// Leds piscando alternado a cada ciclo do loop
bool led_state = false;
double ErroCont = 0.00;
bool OBS = true;

// MQTT
#define MSG_BUFFER_SIZE 50
//char msg[MSG_BUFFER_SIZE];
//char MQTT[20];

/*
  ===============================================================
  ===                      SETUP INICIAL                      ===
  ===============================================================
*/

void setup()
{
  pinMode(LED1, OUTPUT);
  pinMode(LED2, OUTPUT);
  digitalWrite(LED1, led_state);
  digitalWrite(LED2, led_state);
  Serial.begin(9600);
  Setup_MPU();
  Conectar_WiFi();
  Conectar_MQTT();
  delay(500);
  //  Serial.end();
}

/*
  ================================================================
  ===                       MAIN PROGRAM                       ===
  ================================================================
*/

void loop ()
{
  dt = (millis() - tempo_prev) / 1000.0;
  tempo_prev = millis();

  // lê os dados do sensor
  Read_MPU();

  // Converte os dados do sensor
  converterRAW();

  //  // Faz a média do valor após conversão
  AmX = media_movelaX(AcX);
  AmY = media_movelaY(AcY);
  AmZ = media_movelaZ(AcZ);

  // Calcula os ângulos em função dos eixos do sensor
  PitchRollYaw(AmX, AmY, AmZ, GyX, GyY, GyZ);
  //Corrige erro no valor dos angulos
  //  corrigirANGULO();

  //Calculo da velocidade
  velocidade(AmX, AmY, AmZ);

  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  //Mostra os valores
  //  salva("t", tempo_prev);
  //  mostraRAW();
  //  mostraOFF();
  mostraCON();
  //  mostraMED();
  //  mostraANG();
  //  mostraVEL();
  //  salva("dt", dt);
  //  salva("eY", erroYAW);

  //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  finaliza();

  if (!client.connected())
  {
    Reconectar();
  }
  client.loop();

}
